...
class MSRVTTTrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # print(f"处理文本: {caption}")

        # 处理文本
        text_tokens = self._process_text(caption)
        # print(f"处理后的文本: {text_tokens}")
        # print(f"{text_tokens}的数据类型为{type(text_tokens)}")
        # 处理视频
        video_frames = self._process_video(video_id)
        # print(f"处理后的视频: {video_frames}")
        # 确保返回正确的数据类型
        if isinstance(text_tokens, Mapping):
            # print("文本处理结果为字典，将只返回语言部分")
            # text_tokens = text_tokens["language"]
    ...
    def _process_video(self, video_id):
        ...
        try:
            ...
            # print(f"video_tensor is: {video_tensor}")
            # print(f"视频tensor形状: {video_tensor.shape}")
            # print(f"video_tensor unique is: {torch.unique(video_tensor)}")
            return video_tensor
